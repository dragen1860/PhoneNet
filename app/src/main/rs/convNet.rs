#pragma version(1)
#pragma rs java_package_name(com.openvw.phonenet)
...
float RS_KERNEL reshape(uint32_t x) {
    //Wout = Hout = 227
    //exe space : 227 * 227
    int32_t w = x % Wout;
...
float RS_KERNEL conv_1(uint32_t x) {
     // x : output dimension
	int32_t w = (x / 4) % Wout;           //width idx
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout; //height idx
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4; //layers idx

    //in (N,Hin,Win) with kernel(M,N,K,K) => (M,Hout,Wout)
    //current idx of out: (m, h, w)
	int32_t n, i, j;

	float out0 = rsGetElementAt_float(bias, m);

	for (n = 0; n < N_new; n++){ // N_new = inputlayers /4
		for (i = 0; i < K; i++)  {
			for (j = 0; j < K; j++){

			    //j-K : kernel width idx
			    //i-K : kernel height idx
			    //n-N_new : input layers idx_4
			    //w : output width idx
			    //h : output height idx
			    //m : output layers idx, kernel layer idx

			    //idx:  (n,h*S+j,w*S+i) with (m,4*n,i,j)  => (m,   h,  w)
			    //size: (N,Hin,Win)     with (M, N, K, K) => (M, Hout, Wout)
				if (w*S + i - pad<0 || w*S + i - pad >= Win ||h*S + j - pad<0 || h*S + j - pad >= Hin) continue;

                int32_t in_idx = (w * S + i - pad) + Win * (h * S + j - pad) + (Win * Hin * n);
				float4 in0 = rsGetElementAt_float4(in, in_idx);

				int32_t w_idx = (i) + (K * j) + (K * K * n);
				float4 wght0 = rsGetElementAt_float4(weight, w_idx + ((K * K * N_new) * m));
	...
	for (n = 0; n < N_new; n++){ // N_new = inputlayers /4
		for (i = 0; i < K; i++)  {
			for (j = 0; j < K; j++){
				...
				int32_t w_idx = (i) + (K * j) + (K * K * n) + ((K * K * N_new) * m);
				float4 wght0 = rsGetElementAt_float4(weight, w_idx);
...
float RS_KERNEL conv_2(uint32_t x) {
	int32_t w = (x / 4) % Wout;
...
float RS_KERNEL conv_4(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_6(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_8(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_12(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_16(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_20(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_24(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_28(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_32(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_36(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_40(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_44(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_48(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
...
float RS_KERNEL conv_64(uint32_t x) {
	int32_t w = (x / 4) % Wout;
	int32_t h = (((x - 4 * w) / 4) / Wout) % Hout;
	int32_t m = (x % 4) + (x / (4 * Wout * Hout)) * 4;
